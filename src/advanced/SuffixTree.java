...
class SuffixTree {
    ...
    private void addChildNode(Node parentNode, String text, int index) {
        parentNode.children.add(new Node(text, index));
    }
    ...
    private void splitNodeToParentAndChild(Node parentNode, String parentNewText, String childNewText) {
        Node childNode = new Node(childNewText, parentNode.positions);

        if (parentNode.children.size() > 0) {
            while (parentNode.children.size() > 0) {
                childNode.children
                        .add(parentNode.children.remove(0));
            }
        }

        parentNode.children.add(childNode);
        parentNode.text = parentNewText;
        parentNode.positions = POSITION_UNDEFINED;
    }
